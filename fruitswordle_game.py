...
# 4. ฟอร์มรับคำตอบที่เสถียรที่สุด (ล้างกล่องพิมพ์ให้อัตโนมัติด้วย clear_on_submit)
if not st.session_state.เกมจบแล้ว and รอบปัจจุบัน < โอกาสทั้งหมด:
    with st.form(key="wordle_form", clear_on_submit=True):
        # รับค่าและเก็บไว้ในคีย์ชั่วคราวที่เป็นอิสระในแต่ละรอบ เพื่อป้องกันค่าหาย
        เดาคำ = st.text_input(
            f"พิมพ์คำศัพท์ 5 ตัวอักษร (เหลือโอกาส {โอกาสทั้งหมด - รอบปัจจุบัน} ครั้ง):",
            max_chars=5,
            key=f"round_input_{รอบปัจจุบัน}",
        ).lower().strip()

        ปุ่มกด = st.form_submit_button("ทายเลย")

    # ประมวลผลเมื่อกดปุ่มทาย หรือกด Enter
    if ปุ่มกด:
        if not เดาคำ:
